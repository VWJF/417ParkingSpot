...
parking_map.prototype.find_parking_map_for_current_location = function(reserveMenuStringBuilder, hourly_rate)
{
	...
	if(navigator.geolocation) {
		...
		navigator.geolocation.getCurrentPosition(function(position) {
			...
			current.load_parking_spots_onto_map(reserveMenuStringBuilder, hourly_rate);
...
parking_map.prototype.load_parking_spots_onto_map = function(contentStringBuilder, hourly_rate_val)
{
	var current = this;
	...
	$.ajax({
		...
		dataType: 'json',
		type: 'post',
		data: { request_type: "ALL", hourly_rate: hourly_rate_val },
...
parking_map.prototype.find_parking_spots_nearby_address = function(address_val, reserveMenuStringBuilder, hourly_rate) {
	...
	current.geocoder.geocode( { 'address': current.selected_address}, function(results, status) {
		...
		if (status == google.maps.GeocoderStatus.OK) {
			var position_val = results[0].geometry.location;
			current.map.setCenter(position_val);
			current.load_parking_spots_onto_map(reserveMenuStringBuilder);
